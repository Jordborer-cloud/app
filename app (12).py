...
with col1:
    ...
    annual_draw_per_child = st.number_input('Annual Drawdown per Child (ZAR)', min_value=0, value=500_000, step=50_000, format="%d")
...
with col2:
    ...
    # Plot a few random simulation paths
...
st.markdown("This simulation uses a Monte Carlo approach to estimate how long a lump sum investment can sustain annual drawdowns across multiple generations.")

st.markdown("- The model begins with a fixed investment amount and applies annual real returns drawn from a normal distribution based on your selected mean and volatility.")
st.markdown("- Each year, a fixed drawdown per descendant is subtracted from the portfolio.")
st.markdown("- The simulation runs over a generation length, defined as life expectancy minus 30 years.")
st.markdown("- The simulation is repeated many times to show a range of outcomes.")
st.markdown("- The model assumes all descendants begin drawing from the first year (i.e., no staggered births).")

st.markdown("It tracks:")
st.markdown("1. How many years the portfolio lasts before it is depleted.")
st.markdown("2. What the ending value of the portfolio is at the end of the time period.")
